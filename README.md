...
`@metrics/module-a` and `@metrics/module-b` depend on the same `3rd-party-dependency`.

`@metrics/module-a` is a public facing module that other modules can use as a dependency. `@metrics/module-b` is __not__ a public facing module. Rather, it's an internal module which contains code that is only of interest to other modules within our organisation and it should never be used as a dependency by external modules.

If we were to use the default configuration from Renovate Bot the following would happen when we have Semantic Release in our setup and there is a new version of the `3rd-party-dependency`: `@metrics/module-a` would get a PR with an update of the `3rd-party-dependency` and when merged a new release of `@metrics/module-a` would be cut. Meanwhile `@metrics/module-b` would also get a PR with an update of the `3rd-party-dependency` and when merged a new release of `@metrics/module-b` would also be cut. 

This release of `@metrics/module-b` would again create a new PR in `@metrics/module-a` and cause a new release of `@metrics/module-a` when merged. 

In other words; Due to both `@metrics/module-a` and `@metrics/module-b` depending on `3rd-party-dependency` an update of `3rd-party-dependency` will cause two releases of `@metrics/module-a` which is not optimal.
...
```json
{
  "$schema": "https://docs.renovatebot.com/renovate-schema.json",
  "extends": [
    "github>metrics-js/renovate-presets:sub-level-module"
  ]
}
```
